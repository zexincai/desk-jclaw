...
function loadWindowState() {
  try {
    if (fs$1.existsSync(stateFilePath)) {
      const data = fs$1.readFileSync(stateFilePath, "utf-8");
...
function saveWindowState() {
  ...
  try {
    ...
    const dir = require$$1.dirname(stateFilePath);
    if (!fs$1.existsSync(dir)) {
      fs$1.mkdirSync(dir, { recursive: true });
    }
    fs$1.writeFileSync(stateFilePath, JSON.stringify(state, null, 2));
  } catch (error2) {
...
function ensureVisibleOnScreen(state) {
  ...
  const visible = displays.some((display) => {
    ...
    return state.x !== void 0 && state.y !== void 0 && state.x >= area.x && state.y >= area.y && state.x + state.width <= area.x + area.width && state.y + state.height <= area.y + area.height;
  });
  if (!visible) {
    delete state.x;
    delete state.y;
...
let tray = null;
function createTray() {
  const appRoot2 = app.getAppPath();
  const iconFileName = process.platform === "darwin" ? "icon.png" : "icon.ico";
  const iconPath = app.isPackaged ? require$$1.join(appRoot2, "..", "build", iconFileName) : require$$1.join(appRoot2, "build", iconFileName);
...
function setupAutoUpdater() {
  ...
  mainExports.autoUpdater.on("download-progress", (progress) => {
    ...
    const mainWindow2 = getMainWindow();
    if (mainWindow2) {
      mainWindow2.webContents.send("update-progress", progress);
    }
  });
